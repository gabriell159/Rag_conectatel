...
validate_interaction = import_module(".00_contract", __package__).validate_interaction
...
def generate_report(input_path: Path = DEFAULT_INPUT, output_path: Path | None = None) -> dict:
    ...
    for event in events:
        try:
            ...
            contract_errors.append({"trace_id": event.get("trace_id"), "error": str(exc)})
    answers = [event for event in events if event.get("decision") == "ANSWER"]
    escalations = [event for event in events if event.get("decision") == "ESCALATE"]
    report = {
        ...
        "contract": {
            "valid_interactions": len(events) - len(contract_errors),
            "invalid_interactions": len(contract_errors),
            "errors": contract_errors,
        },
        "quality_checks": {
            "answer_with_current_citations": sum(bool(event.get("citations")) for event in answers),
            "answer_total": len(answers),
            "escalate_with_complete_handoff": sum(bool(event.get("handoff")) for event in escalations),
            "escalate_total": len(escalations),
        },
        "known_risks": [
